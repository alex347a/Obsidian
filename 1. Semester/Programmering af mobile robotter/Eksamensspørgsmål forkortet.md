...
- **Why binary for microcontrollers?**  
    Microcontrollers use binary (1/0) as they work with two states (on/off).
    
- **Bit vs Byte?**  
    A bit is a single binary digit, a byte is 8 bits.
    
- **Data in memory?**  
    Binary numbers represent data and instructions.
    
- **Why hexadecimal?**  
    Hex is compact and easier for humans to read than binary.
    
- **Converting in MicroPython?**  
    `bin()`, `hex()`, and `int()` convert between binary, hex, and decimal.
    
- **Store binary?**  
    Use `0b` prefix, e.g., `0b1010`.
...
- **Microcontroller vs Microprocessor?**  
    Microcontrollers have CPU, memory, and peripherals; microprocessors only have CPU.
    
- **Common microcontroller applications?**  
    Robotics, home automation, consumer electronics.
    
- **Examples of microcontrollers?**  
    RP2040, ATmega328, ESP8266, STM32.
    
- **Main components?**  
    CPU, RAM, Flash, EEPROM, GPIO, ADC, DAC.
    
- **Why low power?**  
    Optimized for specific tasks, simpler architecture.
    
- **What is a bus?**  
    A bus connects components, transferring data, addresses, and control signals.
    

---

### **1.3 Microcontroller CPU and Execution**

- **CPU function?**  
    The CPU fetches, decodes, and executes instructions.
    
- **Instruction set?**  
    Defines available operations for the CPU.
    
- **Clock cycle?**  
    Time for a CPU operation; faster cycles mean quicker execution.
    
- **Interrupt?**  
    A signal that stops the current task to handle a high-priority event.
    
- **RP2040 execution?**  
    Dual-core ARM Cortex-M0+ processor executes instructions from Flash.
    
- **Dual-core benefits?**  
    Allows parallel tasks and improves real-time performance.
    

---

### **1.4 Memory in Microcontrollers**

- **Types of memory?**  
    RAM (temporary), ROM (permanent), Flash (non-volatile storage).
    
- **RAM function?**  
    Stores temporary data during execution.
    
- **Flash memory?**  
    Non-volatile memory used for storing code.
    
- **Program storage?**  
    Programs are stored in Flash and executed from RAM.
    
- **Stack memory?**  
    Temporarily stores function data like return addresses and local variables.
    
- **Memory speed impact?**  
    Faster memory (RAM) improves execution speed.
    

---

### **1.5 Microcontroller Architectures**

- **Harvard vs Von Neumann?**  
    Harvard has separate memory for instructions/data; Von Neumann shares memory.
    
- **Harvard advantages?**  
    Faster execution due to separate buses.
    
- **Von Neumann pros/cons?**  
    Simple design, slower due to memory sharing.
    
- **RP2040 architecture?**  
    Von Neumann architecture.
    
- **Harvard speed benefit?**  
    Separate buses allow parallel access, reducing delays.
    
- **Von Neumann bottleneck?**  
    Shared memory slows execution due to delays in accessing data and instructions.
...
- **What is programming?**  
    Creating instructions for computers; essential in robotics and embedded systems for automation and control.
    
- **Best practices for readable code:**  
    Descriptive names, consistent formatting, modular code, comments, avoid repetition (DRY).
    
- **Programming language levels:**
    - **High-level**: Python (easy, slower).
    - **Mid-level**: C (balance).
    - **Low-level**: Assembly (efficient, faster).

- **Language choice impacts:**  
    High-level reduces dev time, low-level offers better performance but takes longer.
    
- **Sequential execution:**  
    Code runs line-by-line unless modified by control flow.
...
- **What is syntax?**  
    The rules for structuring code; ensures error-free execution.
    
- **MicroPython vs Python syntax:**  
    MicroPython is a subset of Python with hardware-specific modules.
    
- **Common syntax errors:**  
    Missing colons, incorrect indentation, typos in module names.
    
- **Compiled vs Interpreted languages:**
    - **Compiled**: Machine code before execution (e.g., C).
    - **Interpreted**: Line-by-line execution (e.g., Python).

- **MicroPython as interpreted:**  
    Allows direct execution on resource-constrained devices.
...
- **What is a variable?**  
    Stores data for use in a program.
    
- **Container vs variable:**  
    Containers hold multiple values (e.g., lists), variables store a single value.
    
- **Declaring variables in MicroPython:**  
    `my_variable = 10`, `my_string = "Hello"`, `my_list = [1, 2, 3]`.
    
- **Naming conventions:**  
    Descriptive names, lowercase with underscores (snake_case), no reserved keywords.
    
- **Variable scope:**
    - **Global**: Accessible throughout.
    - **Local**: Accessible within a function.
...
- **Choosing data types:**  
    Use `int` for whole numbers, `float` for decimals, `str` for text, and `bool` for true/false values.
    
- **Typecasting:**  
    Convert data types using functions like `int()`, `float()`, `str()`.
    
- **Mutable vs Immutable:**
    - Mutable: Lists, dictionaries (can be modified).
    - Immutable: Strings, tuples (cannot be modified).
- **Floating-point numbers:**  
    Be cautious of precision issues in MicroPython due to limited memory.
    
- **Lists vs Tuples:**
    - Lists: Mutable, e.g., `my_list[0] = 10`.
    - Tuples: Immutable, cannot be modified after creation.

#### **2.5 Basic Concepts in Programming (Using Python)**

- **Arithmetic Operators:** 
	- `+`, `-`, `*`, `/`, `%` - Used for basic mathematical operations such as addition, subtraction, multiplication, division, and modulus. 
	
- **Integer (`//`) vs Floating-Point (`/`) Division:** - `//` performs integer division, returning only the quotient. - `/` performs floating-point division, returning a float result.
	
- **Modulus for Even/Odd:** - `%` can be used to check if a number is even or odd.
	
- **Conditional Operators:**
	- == , !=, <, >, <=, >=
	- Used for comparing values and checking conditions
...
- **Compound Assignment Operators:** 
	- `+=`, `-=`, `*=` simplify code by combining an operation with assignment.
	
- **Combining Conditional & Logical Operators:** 
...
- **Combining All Operators:** 
	- Arithmetic, conditional, and logical operators can be combined in a single condition.
	
- **Arithmetic in Loops:** 
	- Arithmetic operators can be used inside loops to perform repetitive calculations. 
	
- **Logical Operators for Filtering:** 
	- Logical operators help filter data or validate user input based on multiple conditions.

#### **2.6 Built-In Data Types and Functions**
- **What are the primary built-in data types in Python?**  
    Python includes several built-in data types like numeric, sequence, text, set, mapping, boolean, and binary types.
...
- **Ordered vs Unordered Data Types:**
    - Ordered: The order of elements matters (e.g., `list`, `tuple`, `str`).
    - Unordered: The order does not matter (e.g., `set`, `dict`).
...
- **Homogeneous vs Heterogeneous Data Types:**
    - Homogeneous: All elements are of the same type (e.g., list of integers).
    - Heterogeneous: Elements can be of different types (e.g., list of integers and strings).

- **Typecasting:**  
    Convert one data type to another using functions like `int()`, `float()`, and `str()`.

- **MicroPython Data Types:**  
    MicroPython supports most standard Python data types, but memory constraints may limit their use.

- **Choosing Data Types:**  
    Use `int` for whole numbers, `float` for decimals, `str` for text, `bool` for true/false 

### Module 3 - Basic IO programming using MicroPython
#### **3.1 Hardware walk-through: Introduces the hardware components used in the course, including:**

- **What is the RP2040, and how is it related to the Raspberry Pi Pico?**  
    The RP2040 is a microcontroller developed by the Raspberry Pi Foundation, and it's the core chip of the Raspberry Pi Pico. It handles the processing and communication with peripherals.
    
- **What are the main features of the RP2040 microcontroller?**
    - Dual-core ARM Cortex-M0+ processor
    - 264KB of RAM
    - 2MB of flash memory
    - I/O protocols: GPIO, SPI, I2C, UART
- **What programming languages can be used to develop applications for the RP2040?**  
    Applications for the RP2040 can be developed using:
    - MicroPython
    - C
    - C++
- **What tools and IDEs have you been using for developing and debugging applications on your Raspberry Pi Pico?**  
    ...
    - Visual Studio Code
    - Arduino IDE (for C/C++ development)
- **What are the power requirements of the Raspberry Pi Pico?**  
    The Raspberry Pi Pico requires:
    - Operating voltage: 3.3V
    - Voltage range: 1.8V to 3.6V
- **Can the Raspberry Pi Pico be powered by batteries for mobile robot applications?**  
    Yes, the Raspberry Pi Pico can be powered by a 3.7V Li-Po battery, making it suitable for mobile robot applications.
    
- **What are the different types of I/O (Input/Output) pins available on the Pico? What are their purposes?**  
    The Pico offers various I/O pins:
    - GPIO pins (for input/output)
    - PWM, I2C, SPI, UART, and ADC capabilities
- **How do GPIO pins work on the Raspberry Pi Pico?**  
    GPIO pins can be configured as either input or output to read signals or send digital signals to other components.
    
- **What is the role of PWM (Pulse Width Modulation) pins in controlling motors and LEDs?**  
    PWM pins allow precise control of the power delivered to components like motors and LEDs, adjusting the duty cycle for speed or brightness control.
    
- **How does the I2C and SPI communication protocol help in connecting sensors to the Raspberry Pi Pico?**
    - I2C: Supports multiple devices on a shared bus
    - SPI: Offers faster communication speeds for connecting external sensors and peripherals
    
- **What are the benefits of using the UART interface on the Raspberry Pi Pico?**  
    UART provides a simple serial communication method, ideal for low-speed data transfer with external devices.
    
- **How do you prevent damage to the Raspberry Pi Pico's GPIO pins when connecting external components?**  
    To avoid damage, ensure proper voltage regulation and use current-limiting resistors or buffers when connecting to GPIO pins.
    
- **What are Pico’s internal sensors (if any), and how can they be used?**  
    The internal temperature sensor can be used to monitor the temperature of the microcontroller.

#### **2.2 Monk Makes Electronics Kit 1 for Pico (lite edition): A kit that includes components like jump wires, a breadboard, and electrical components.**
- **Purpose:**  
    A kit for prototyping with the Raspberry Pi Pico, containing essential components like jumper wires, a breadboard, resistors, LEDs, and sensors.
    
- **Complementing the Raspberry Pi Pico:**  
    Provides components to quickly build and prototype circuits, aiding learning and experimentation with electronics.
    
#### **Breadboard Usage**
- **Purpose:**  
    Temporarily connects components without soldering, simplifying circuit prototyping and modifications.
...
#### **Blinking an LED Circuit**
- **Design:**  
    Connect anode to GPIO pin, cathode to ground through a current-limiting resistor.
    - **Purpose:** Demonstrates basic control of an output device.

#### **Resistors in the Kit**
- **Types & Importance:**  
    Various resistors to control current in circuits, such as limiting current to LEDs to prevent damage.

#### **Button Module**
- **How It Works:**  
    Provides a low/high signal when pressed.
    - **Use in Mobile Robot:** Can trigger actions like movement or stopping.

#### **Light Sensor Circuit**
- **Using an LDR:**  
    LDR changes resistance based on light intensity.
    - **Purpose:** Monitor light levels and control LEDs or motors.

#### **Beginner Projects**
- **Examples:**
    - LED blink circuit
    - Temperature sensor monitor
    - Simple light-sensitive device

#### **Connecting an External Sensor (LDR)**
- **Method:**  
    Connect LDR to GPIO pin with a resistor forming a voltage divider, use the ADC functionality of the Pico to read the sensor value.

#### **Troubleshooting Techniques**
- **Steps:**
    - Check connections and component placement.
    - Verify power supply.
    - Use a multimeter to check for shorts or incorrect voltages.


#### **Flashing the Latest Firmware to the Pico**

- **Why is it important to update the firmware?**  
    Ensures the Pico has the latest bug fixes, improvements, and support for new features and peripherals.
    
- **How to flash new firmware?**  
    Hold the BOOTSEL button on the Pico while connecting it to a computer, then drag and drop the firmware file onto the device.
    
- **Where to find the latest firmware?**  
    Download from the official MicroPython website or the Raspberry Pi Foundation’s website.
    
- **Common issues and solutions:**
    - Issue: Pico not entering boot mode or connection problems.
    - Solution: Ensure the BOOTSEL button is pressed correctly or try different USB cables.
- **How to check if the firmware update was successful?**  
    Verify by checking if the Pico appears as a removable drive and running `import machine` to check the firmware version.
    

#### **Thonny Python IDE**

- **What is Thonny and why is it recommended?**  
    A beginner-friendly IDE for Python, ideal for writing, running, and debugging MicroPython code on the Raspberry Pi Pico.
    
- **How to install Thonny IDE?**  
    Download the installer from the Thonny website and run the installation process.
    
- **How to connect the Pico to Thonny?**  
    Connect the Pico via USB and select the correct interpreter (MicroPython on Raspberry Pi Pico) in Thonny’s settings.
    
- **Basic features of Thonny:**  
    Includes an interactive shell, easy script editing, debugging tools, and built-in support for MicroPython.
    
- **Using Thonny’s REPL for interactive programming:**  
    Enter commands interactively, evaluate expressions, and get immediate feedback on the Pico’s state.
    
- **How to upload and run a Python script?**  
    Write the script, save it, and click "Run" to upload and execute it on the Pico.
    
- **Common debugging techniques in Thonny:**  
    Use breakpoints, print statements, or the debugger to step through code and identify issues.
    
- **Using Thonny’s debugger:**  
    Step through code line by line, check variables, and inspect the program’s state during execution.

- **What is MicroPython, and why is it suitable for embedded systems?**  
    MicroPython is a lightweight version of Python optimized for microcontrollers, with a small memory footprint and hardware interaction capabilities.
    
- **Differences between MicroPython and standard Python:**
    - MicroPython: Optimized for microcontrollers with a smaller memory footprint.
    - Standard Python: Designed for desktop environments with more comprehensive features.
- **How does MicroPython control hardware components like LEDs and sensors?**  
    MicroPython provides libraries to interact directly with GPIO pins and peripherals, enabling control of components like LEDs and sensors.
    
- **MicroPython standard libraries vs regular Python libraries:**  
    MicroPython libraries are lightweight and optimized for embedded systems, whereas regular Python libraries are designed for resource-heavy desktop environments.
    
- **Standard libraries used in the course:**  
    `machine`, `time`, `utime`, `math`, and others.
    
- **Key MicroPython modules for hardware interaction:**
    - `machine`: For hardware interaction.
    - `time`: For timing operations.
    - `utime`: For high-precision time management.
    
- **How does the machine module interact with GPIO pins?**  
    The `machine` module provides the `Pin` class for configuring GPIO pins as inputs or outputs, enabling interaction with external components.
    
- **What is the Pin class and how is it used?**  
    The `Pin` class represents a GPIO pin, allowing configuration as input or output and interaction with external components.
    
- **How can the Timer module create scheduled tasks?**  
    The `Timer` module allows setting up timed tasks to run after a specific interval or repeatedly for time-based actions.
    
- **What is the utime module and how does it help with time-based operations?**  
    The `utime` module provides functions for high-precision time measurements and sleep operations, essential for controlling event timing.
    
- **What is the PWM class and how is it used?**  
    The `PWM` class in the `machine` module controls pulse width modulation, adjusting the speed of motors or brightness of LEDs.

#### **Key Modules, Classes, and Functions**

- **Adjusting RP2040’s clock speed for performance and energy efficiency:**  
    Use the `machine.freq()` function to adjust the clock speed, balancing performance and power consumption.
    
- **Connecting a simple LED circuit to the Raspberry Pi Pico:**  
    Connect the LED’s anode to a GPIO pin and the cathode to ground through a current-limiting resistor for basic GPIO control.
...
```
#### **Pull-up and Pull-down Resistors in Button Circuits**

- **Purpose:**  
    Pull-up and pull-down resistors ensure a known voltage level on a GPIO pin when a button is pressed or not pressed, preventing the pin from floating (picking up random signals).
    
- **Pull-up Resistor:**
    - Connects the pin to a high voltage (e.g., 3.3V).
    - When the button is not pressed, the pin reads high (logic 1).
    - When the button is pressed, the pin is connected to ground, reading low (logic 0).
    
- **Pull-down Resistor:**
    - Connects the pin to ground.
    - When the button is not pressed, the pin reads low (logic 0).
    - When the button is pressed, the pin is connected to high voltage, reading high (logic 1).
- **Importance:**  
    These resistors prevent floating inputs and ensure reliable button readings.
...
- **polarity** and **phase**: These define the clock signal’s behaviour (refer to the device’s datasheet for correct settings).
...
- **Object-Oriented Programming (OOP):**  
    OOP is a programming paradigm that organizes code into objects, which are instances of classes. It promotes modularity and code reuse by encapsulating data (attributes) and behaviour (methods).
...
- This can be considered as polymorphism because the same operator behaves differently depending on the operand types.
  Example:
  Adding two vectors can be defined so the x-coordinates and y-coordinates are added, which then means the + operator has been customised for vectors.
...
4. **MicroPython Script to Rotate a Servo to 0°, 90°, and 180°**:
   ...
   def set_servo_angle(angle):
       duty = int((angle / 180) * 1023 + 40)  # Convert angle to PWM duty cycle (between 40 and 115 out of 1023)
       servo.duty(duty)
...
| ------------------ | ------------------------------ | ----------------------------------- |
| Roller Orientation | Angled (e.g., 45°)             | Perpendicular wheel axis            |
...
- Can be disabled in the code without having to remove the lines of code for logging.
Example:
``` Python
import logging
...
- Correlates system behaviour with logged events.
...
- Highlights inconsistencies or unexpected behaviour in datasets.
...
- **Reduced Crosstalk**: Differential signalling in USB eliminates interference, making it robust for high-speed data transfer.
...
I2C (Inter-Integrated Circuit): A two-wire serial protocol for short-distance communication between microcontrollers and peripherals. (half-duplex)
...
SDA and SCL lines are open-drain, meaning they need external pull-up resistors to ensure proper voltage levels when no device is driving the lines. This is because they cant pull the lines high, but they can pull them low, so a pull-up resistor insures the lines are always high until they are pulled low by one of the devices.
...
**7-Bit Addressing**:
- Supports up to 127 unique devices (technically around 112 because the last ones are reserved for protocol-specific functions).
**10-Bit Addressing**:
- Supports up to 1023 unique devices (technically less), used for more complex systems.
...
4. **Stop Condition**:
    - The master "releases" which pulls SCL high and thereafter SDA high.
...
- **Full-Duplex Communication:** Data can be sent and received simultaneously. (I2C only supports half-duplex)
...
CPOL (Clock Polarity):
CPOL = 0 (Leading Edge: RISING), meaning idle is low and active is high
CPOL = 1 (Leading Edge: FALLING), meaning idle is high and active is low

CPHA (Clock Phase)
CPHA = 0 Data samples on leading edge
CPHA = 1 Data samples on trailing edge
![[SPI CPHA (Clock Phase).png]]
