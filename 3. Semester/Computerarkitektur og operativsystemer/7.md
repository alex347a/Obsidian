...
A CPU core can only do one operation at a time, so if the process has to wait the CPU executes a different ready operation and the waiting process values are stored in the PCB.