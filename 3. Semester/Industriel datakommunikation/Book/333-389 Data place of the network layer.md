...
- **Flow Labeling:** A field to label packets belonging to the same "flow" (a series of packets from a source to a destination) for which special handling may be required.
...
- **Firewalls:** Sit at the network boundary and inspect packet headers. They can block packets based on IP addresses, port numbers, or even the state of TCP connections. They are a first line of defense.
